...
async def correct_user_misspell(client: Client, context: Context, command: str, *args) -> None:
	"""
	...
	Args:
		...
		command_type ():
		replace (dict or string): arguments to replace
	...
	try:
		...
		context.message.content = f"{command_prefix} {command} {' '.join(args)} [modified]" # change cli inputs
